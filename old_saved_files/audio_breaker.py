...
def break_file3(audio_dpath: Union[str, Path], txt_fpath, total_time_in_sec):
...
def generate_labels(audios: List[Dict], n_jobs=6):
    ...
    db_ = DBSCAN(eps=.5, min_samples=4, metric='euclidean', n_jobs=n_jobs)
...
def audio_repeater(audio):
    ...
    num_repeats = int(np.ceil(5 / duration))
...
test_translation_v1(args.audio_path, args.srt_path, n_jobs=args.n_jobs, wav_fpath=args.out, cache_address=args.cache_address, audio_cache_address=args.audio_cache_address, audios_cache='audios.pkl')
